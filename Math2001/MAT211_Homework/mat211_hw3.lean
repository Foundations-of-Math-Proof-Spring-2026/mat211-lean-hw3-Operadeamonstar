...
"extra" proves relations between a LHS and a RHS differing by some neutral quantity for the relation
-/
...
example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 :=
  calc
    ...
    _ = (3 * n ^ 3 + 7 * n ^ 2* n - 2 * n ^ 2) := by ring
    _ ≥ (3 * n ^ 3 + 7 * n ^ 2 * 10 - 2 * n ^ 2) := by rel [hn]
